...
io.on('connection', (socket) => {
    ...
    socket.on('host-join', (data) => {
        ...
        MongoClient.connect(url, 'useUnifiedTopology: true', function(err, db) {
            if (err) throw err;
            var dbo = db.db("PregunteroDB");
            var query = { id: parseInt(data.id) };
            dbo.collection('Preguntas').find(query).toArray(function(err, result) {
                if (err) throw err;

                //A kahoot was found with the id passed in url
                if (result[0] !== undefined) {

                    var gamePin = Math.floor(Math.random() * 3 * 4 * 9 / 4 - 2) + 26; //new pin for game
    ...
    socket.on('host-join-game', (data) => {
        ...
        if (game) {
            ...
            MongoClient.connect(url, function(err, db) {
                ...
                dbo.collection("Preguntas").find(query).toArray(function(err, res) {
                    if (err) throw err;
                    ...
                    socket.emit('gameQuestions', {
                        ...
                        questionsCount: res[0].questions.length
                    });
                    db.close();
                });
            });
        ...
        if (player) {
            ...
            MongoClient.connect(url, function(err, db) {
                ...
                dbo.collection("Preguntas").find(query).toArray(function(err, res) {
                    ...
                    var answer4 = res[0].questions[0].answers[3];
                    var correctAnswer = res[0].questions[0].correct;
                    var feedback = res[0].questions[0].feedback;
            ...
            MongoClient.connect(url, function(err, db) {
                ...
                dbo.collection("Preguntas").find(query).toArray(function(err, res) {
                    ...
                    if (num == correctAnswer) {
                        ...
                        // io.to(game.pin).emit('getTime', socket.id); //VARIA PUNTAJE DE ACUERDO AL TIEMPO QUE TARDO EN RESPONDER
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            dbo.collection("Preguntas").find(query).toArray(function(err, res) {
                ...
                if (res[0].questions.length >= game.gameData.question) {
                    ...
                    var feedback = res[0].questions[questionNum].feedback;

                    io.emit('gameQuestions', {
                        q1: question,
                        a1: answer1,
                        a2: answer2,
                        a3: answer3,
                    ...
                    for (var i = 0; i < playersInGame.length; i++) {
                        ...
                        if (playersInGame[i].gameData.score == "") {
                            playersInGame[i].gameData.score = 0;
                        }
    ...
    socket.on('requestDbNames', function() {
        ...
        MongoClient.connect(url, function(err, db) {
            ...
            var dbo = db.db('PregunteroDB');
            dbo.collection("Preguntas").find().toArray(function(err, res) {
                if (err) throw err;
    ...
    socket.on('newQuiz', function(data) {
        MongoClient.connect(url, function(err, db) {
            ...
            dbo.collection('Preguntas').find({}).toArray(function(err, result) {
                if (err) throw err;
                var lengthCuestionario = Object.keys(result).length;
                var lastId = result[lengthCuestionario - 1].id;
                var newId = lastId + 1;

                data.id = newId;

                var game = data;
                dbo.collection("Preguntas").insertOne(game, function(err, res) {
                    if (err) throw err;
                    db.close();
                });
                db.close();
